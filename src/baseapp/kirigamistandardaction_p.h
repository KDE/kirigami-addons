...
struct KirigamiStandardActionInfo {
    StandardAction id;
...
// clang-format off
static const KirigamiStandardActionInfo g_rgActionInfo[] = {
...
inline const KirigamiStandardActionInfo *infoPtr(StandardAction id)
{
...
public Q_SLOTS:
    ...
    void invokeEditSlot(const char *slot)
    {
        if (qGuiApp->focusWindow()) {
            const auto window = qGuiApp->focusWindow();
            auto quickWindow = qobject_cast<QQuickWindow *>(window);
            if (quickWindow && quickWindow->activeFocusItem()) {
                QMetaObject::invokeMethod(quickWindow->activeFocusItem(), slot);
            }
        }
    }
};
